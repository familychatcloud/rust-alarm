...
Android includes an English voice from the open-source Flite engine as a fallback if no usable system text-to-speech engine is available. Voices depend on installed languages. Optional keep-alive mode is off by default and may use more battery.
...
Arabic (Egypt), Bangla (Bangladesh), Dutch (Netherlands), English (India), American English, French (France), German (Germany), Hindi (India), Indonesian (Indonesia), Italian (Italy), Japanese (Japan), Korean (South Korea), Marathi (India), Polish (Poland), Brazilian Portuguese, Romanian (Romania), Russian (Russia), Spanish (United States), Tamil (India), Telugu (India), Thai (Thailand), Turkish (Türkiye), Ukrainian (Ukraine) and Vietnamese (Vietnam).
...
This repository is a product-information page, not a distribution of the proprietary app source. The bundled Flite speech engine is open source.
