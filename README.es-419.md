...
<!-- section:comparison -->
## ¿Por qué elegir este despertador en Rust en vez de Alarmy?

Rust combina rendimiento nativo y seguridad de memoria, y permite compartir el núcleo de la app entre iOS y Android. Esto ayuda a mantener Noler pequeño y confiable, sin bibliotecas de anuncios, SDK de analítica ni servicios de clima, horóscopos o sueño. Usar Rust no demuestra por sí solo un ahorro de batería. Nuestro diseño combina código nativo con la eliminación de tareas en segundo plano ajenas a las alarmas; las cifras de consumo requieren pruebas en dispositivos físicos.
...
| **Enfoque de batería** | El ahorro de batería es una prioridad del producto. El funcionamiento normal de la alarma está completamente fuera de línea, lo que evita el funcionamiento de Wi-Fi/datos celulares además del uso de la batería causado por el clima, el horóscopo, el análisis del sueño, los anuncios, los análisis y la sincronización en la nube. | Su conjunto más amplio de funciones conectadas puede realizar trabajos de red y procesamiento más allá de programar y hacer sonar alarmas. |
...
Únicamente en Android, el motor de código abierto Flite incluido ofrece una voz en inglés si no hay un motor de voz del sistema que se pueda usar. Las voces dependen de los idiomas instalados. El modo opcional que mantiene la app activa viene apagado y puede gastar más batería.
...
árabe (Egipto), bengalí (Bangladés), neerlandés (Países Bajos), inglés (India), inglés estadounidense, francés (Francia), alemán (Alemania), hindi (India), indonesio (Indonesia), italiano (Italia), japonés (Japón), coreano (Corea del Sur), maratí (India), polaco (Polonia), portugués de Brasil, rumano (Rumania), ruso (Rusia), español (Estados Unidos), tamil (India), telugu (India), tailandés (Tailandia), turco (Turquía), ucraniano (Ucrania) y vietnamita (Vietnam).
...
Este repositorio contiene información del producto, no el código propietario de la aplicación. El motor de voz Flite incluido es de código abierto.
